      ...
      if (IsAdmin(User) || IsRootAdmin(User) || m_OutPackets.size() < 9)
      {
        switch (CommandHash)
        {
          ...
          case HashCode("getgame"):
          {
            ...
            {
              if (m_Aura->m_CurrentGame)
                QueueChatCommand("[" + m_Aura->m_CurrentGame->GetDescription() + "] in lobby " + to_string(m_Aura->m_Games.size()) + "/" + to_string(m_Aura->m_MaxGames) + " in progress", User, Whisper, m_IRC);
              else
                QueueChatCommand("" + to_string(m_Aura->m_Games.size()) + "/" + to_string(m_Aura->m_MaxGames) + " in progress", User, Whisper, m_IRC);
              break;
            }
